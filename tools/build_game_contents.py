...
def write_curated(waves):
    """Scenarios the app can offer but the card database has never heard of.

    Civil War and She-Hulk are played as a pair — a side, Resistance or
    Registration, plus the hero you face — and MarvelCDB has entered neither.
    Without this the app cannot offer them at all, and with it the app can name
    the game to play while admitting it has no cards to show for it.

    Codes are invented here and must stay stable: a saved play or a history row
    refers to one. Derived from the English name so they are at least legible.
    """
    scenarios = []
    for wave in waves:
        for pack in wave["packs"]:
            special = pack.get("special")
            if not special:
                continue
            for entry in pack["scenarios"]:
                if entry["code"]:
                    # MarvelCDB has it; the real code wins.
                    continue
                low, high = special["modularCount"]
                scenarios.append({
                    "code": f"{pack['code']}_{slug(entry['en'])}",
                    "packCode": pack["code"],
                    "modularCountMin": low,
                    "modularCountMax": high,
                    "modularPacks": special["modularsRestrictedTo"],
                    "names": {"en": entry["en"], "fr": entry["fr"]},
                })
    ...
    restricted = sorted({
        ...
        for wave in waves
        for pack in wave["packs"]
        for code in pack.get("special", {}).get("modularsRestrictedTo", [])
    })

    document = {
        "_note": (
            "Generated by tools/build_game_contents.py from docs/game_contents.json. "
            "Scenarios the game has and MarvelCDB has not entered, so the randomiser can "
            "offer them; the app has no card codes for these and cannot show a setup. "
            "restrictedModularPacks names packs whose modular sets belong to their own "
            "scenarios and must not be drawn for anybody else's."
        ),
        "restrictedModularPacks": restricted,
        "scenarios": scenarios,
    }
    with io.open("app/src/main/assets/curated_scenarios.json", "w",
                 encoding="utf-8", newline="\n") as out:
        json.dump(document, out, ensure_ascii=False, indent=2)
        out.write("\n")
    print("curated scenarios:", len(scenarios), "| restricted modular packs:", restricted)
